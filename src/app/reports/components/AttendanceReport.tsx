...
const AttendanceReport: React.FC<AttendanceReportProps> = ({
  ...
  const getEnhancedMonthlySummary = (): MonthlySummary => {
    ...
    const totalCount = presentDays + weekoffDays + halfDays + lopDays;
    ...
    doc.text('Employee Signature', 125, signatureY + 6);
      ...
      try {
        ...
        if (leaveData.leaveHistory?.length > 0) {
          const filteredLeaveHistory = filterLeaveHistoryByMonth(
          ...
          if (filteredLeaveHistory.length > 0) {
            ...
            // Modified headers and data to include half-day information
    ...
    <div className="space-y-6 bg-white rounded-xl shadow-sm p-6">
      ...
      {loading ? (
        ...
        <>
          ...
          {/* Attendance Monthly Summary (custom block) */}
          {summary && (
            <div className="mt-2">
              <h3 className="text-lg font-bold text-gray-800 mb-2">Attendance Monthly Summary</h3>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                {(() => {
                  const totalDaysInMonth = new Date(selectedYear, selectedMonth, 0).getDate();
                  const workingDays = summary.workingDays || 0;
                  const weekoffHoliday = totalDaysInMonth - workingDays;
                  const presentDays = summary.presentDays || 0;
                  const leaveBreakdown = getLeaveBreakdownFromHistory();
                  const totalLeaveDays = (leaveBreakdown['SL'] ?? 0) + (leaveBreakdown['CL'] ?? 0) + (leaveBreakdown['EL'] ?? 0) + (leaveBreakdown['CompOff'] ?? 0);
                  const absentOrLOP = Math.max((summary.absentDays !== undefined ? summary.absentDays : (summary.data?.absentDays ?? 0)) - totalLeaveDays, 0);
                  const halfDayLeave = leaveBreakdown.halfDayLeave || 0;
                  return (
                    <>
                      <div className="grid grid-cols-2 md:grid-cols-9 gap-4 text-center">
                        <div>
                        ...
                        <div>
                          ...
                          <div className="text-xl font-bold text-blue-600">{workingDays}</div>
                        </div>
                        <div>
                          <div className="text-xs text-gray-500">Weekoff/Holiday</div>
                          <div className="text-xl font-bold text-blue-600">{weekoffHoliday}</div>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <div className="text-xl font-bold text-yellow-600">{leaveBreakdown['SL'] ?? 0}</div>
                        </div>
                        <div>
                          <div className="text-xs text-gray-500">CL</div>
                          <div className="text-xl font-bold text-yellow-600">{leaveBreakdown['CL'] ?? 0}</div>
                        </div>
                        <div>
                          <div className="text-xs text-gray-500">EL</div>
                          <div className="text-xl font-bold text-yellow-600">{leaveBreakdown['EL'] ?? 0}</div>
                        </div>
                        <div>
                          <div className="text-xs text-gray-500">CompOff</div>
                          <div className="text-xl font-bold text-yellow-600">{leaveBreakdown['CompOff'] ?? 0}</div>
                        </div>
                        <div>
                          <div className="text-xs text-gray-500">Half Day Leave</div>
                          <div className="text-xl font-bold text-purple-600">{halfDayLeave}</div>
                        </div>
                        <div>
                          <div className="text-xs text-gray-500">Absent/LOP</div>
                          <div className="text-xl font-bold text-red-600">{absentOrLOP}</div>
                        </div>
                      </div>
                      <div className="mt-3 text-sm">
                        <span className="text-green-600 font-semibold">Summary matches API and leave history.</span>
                      </div>
                    </>
                  );
                })()}
            ...
            <div className="mt-8">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody>
                    {Object.entries(leaveBalance.balances).map(([type, values]) => {
                      const v = values as { allocated: number; used: number; remaining: number; pending: number };
                      return (
                        <tr key={type}>
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{type}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{v.allocated}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{v.used}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{v.remaining}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{v.pending}</td>
                        </tr>
                      );
                    })}
                  </tbody>
              ...
              <div className="mt-2 text-sm text-gray-600">
                <strong>Total Allocated:</strong> {leaveBalance.totalAllocated} &nbsp;
                <strong>Total Used:</strong> {leaveBalance.totalUsed} &nbsp;
                <strong>Total Remaining:</strong> {leaveBalance.totalRemaining} &nbsp;
                <strong>Total Pending:</strong> {leaveBalance.totalPending}
              </div>
            </div>
          )}
        </>
